...
const DeletionLogModel = mongoose.model('DeletionLog5000000', deletionLogSchema);
...
export async function generateExpiryDocuments(count) {
  ...
  for (let i = 0; i < count; i++) {
    const expireAt = new Date(currDate + Math.floor(Math.random() * 600000) + 60000*20); // Random time within the next 10 minutes
    const expiryDoc = new ExpiryModel({
      value: `Document ${i + 1}`,
      expireAt: expireAt
    });
    documents.push(expiryDoc);
    if(documents.length > 1000000){
